...
export default function QuestionsPage() {
  ...
  const add = async () => {
    ...
    await fetch('/api/questions', {
      ...
      body: JSON.stringify({ module_id: moduleId, type, text })
    })
    resetForm()
    load()
  }
  ...
  const update = async () => {
    ...
    await fetch(`/api/questions/${selectedQuestion.question_id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ module_id: moduleId, type, text })
    })
      ...
      <div className="flex flex-col gap-2 mb-6">
        ...
        {selectedQuestion ? (
          <div className="flex gap-2">
            <button className="px-4 py-2 rounded bg-blue-600" onClick={update}>Update Question</button>
            <button className="px-4 py-2 rounded bg-gray-500" onClick={resetForm}>Cancel</button>
          </div>
        ) : (
      ...
      <ul className="space-y-2">
        {items.map(q => (
          <li key={q.question_id} className="flex items-center justify-between border rounded px-3 py-2">
            <div>
              #{q.question_id} — [{q.type}] {q.text}{" "}
              <span className="opacity-70">({q.module?.name ?? q.module_id})</span>
            </div>
