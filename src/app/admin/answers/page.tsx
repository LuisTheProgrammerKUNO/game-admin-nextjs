...
export default function AnswersPage() {
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`/api/answers/${id}`, { method: 'DELETE' }); load()
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <input className="border px-2 py-1 flex-1" value={text} onChange={e=>setText(e.target.value)} placeholder="Answer text" />
      ...
      <ul className="space-y-2">
        {answers.map(a => (
          <li key={a.answer_id} className="flex items-center gap-3">
            ...
            <button className="border px-2 py-0.5" onClick={()=>toggle(a)}>{a.is_correct ? 'Mark wrong' : 'Mark correct'}</button>
            <button className="border px-2 py-0.5" onClick={()=>remove(a.answer_id)}>Delete</button>
          </li>
        ))}
      </ul>
    </div>
